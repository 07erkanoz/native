...
### 2.8 Kişi Yönetimi (Kapsamlı)

#### 2.8.1 Cihaz Rehber Hesapları

Cihazda kayıtlı tüm hesaplar (Google, Samsung, Telefon, WhatsApp vb.) gruplar halinde gösterilir:

```typescript
// Cihaz hesapları
interface DeviceAccount {
  id: string;
  type: 'google' | 'samsung' | 'phone' | 'whatsapp' | 'telegram' | 'other';
  name: string;                    // "example@gmail.com"
  displayName: string;             // "Google - example@gmail.com"
  icon: string;                    // Hesap ikonu
  contactCount: number;            // Bu hesaptaki kişi sayısı
  isVisible: boolean;              // Listede gösterilsin mi
  canAdd: boolean;                 // Bu hesaba kişi eklenebilir mi
  canEdit: boolean;                // Bu hesaptaki kişiler düzenlenebilir mi
  canDelete: boolean;              // Bu hesaptaki kişiler silinebilir mi
}

// Hesapları çekme
async function getDeviceAccounts(): Promise<DeviceAccount[]> {
  const accounts = await Contacts.getContactAccounts();

  return accounts.map(acc => ({
    id: `${acc.type}_${acc.name}`,
    type: mapAccountType(acc.type),
    name: acc.name,
    displayName: `${acc.type} - ${acc.name}`,
    icon: getAccountIcon(acc.type),
    contactCount: 0,  // Sonra hesaplanacak
    isVisible: true,
    canAdd: acc.type !== 'whatsapp',  // WhatsApp'a kişi eklenemez
    canEdit: true,
    canDelete: true
  }));
}
...
│  HESAPLAR                                               ▼   │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ [✓] 📧 Google (example@gmail.com)       1,245 kişi │   │
│  │ [✓] 📱 Telefon                            156 kişi │   │
│  │ [ ] 💬 WhatsApp                           432 kişi │   │
│  │ [✓] 🟢 Samsung (example@samsung.com)       23 kişi │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  ─────────────────────────────────────────────────────────  │
│                                                          #  │
...
│  │ AK   │  Ali Kaya                              📱        │
│  └──────┘  0536 xxx xx xx                                  │
│  (Otomatik avatar - fotoğraf yok)                          │
│                                                             │
│  B                                                          │
│  ┌──────┐                                                   │
│  │ IMG  │  Barış Koçkaya                         📧        │
│  └──────┘  +90 542 xxx xx xx                               │
│  (Cihaz rehberindeki fotoğraf gösteriliyor)                │
│                                                             │
├─────────────────────────────────────────────────────────────┤
│  [+]              KİŞİLERİ ARA                    [⋮]      │
└─────────────────────────────────────────────────────────────┘
...
#### 2.8.3 Yeni Kişi Ekleme (Gelişmiş)
...
│  │    ─────────────────────────────────────────────   │   │
│  │    📧 Google (example@gmail.com)     ✓           │   │
│  │    📱 Telefon (yerel depolama)                    │   │
│  │    🟢 Samsung (example@samsung.com)               │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  ─────────────────────────────────────────────────────────  │
│                                                             │
...
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  📞 TELEFON NUMARALARI                        [+ Ekle]     │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ Mobil          ▼    [+90                        ] │   │
│  │ ─────────────────────────────────────────────────  │   │
│  │ Mobil  │ Ev  │ İş  │ Ana  │ İş Faks │ Ev Faks  │  │   │
│  │ Çağrı Cihazı │ Diğer │ Özel...                   │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  📧 E-POSTA ADRESLERİ                         [+ Ekle]     │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ Kişisel        ▼    [                           ] │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
...
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  🎂 ÖNEMLİ TARİHLER                           [+ Ekle]     │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ Doğum Günü      ▼    [Tarih Seç              📅] │   │
│  │ Yıldönümü       ▼    [Tarih Seç              📅] │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  🌐 WEB SİTESİ & SOSYAL MEDYA                 [+ Ekle]     │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ Web Sitesi          [https://                   ] │   │
│  │ LinkedIn            [                           ] │   │
│  │ Twitter             [                           ] │   │
...
│  │ Eş               ▼  [Kişi Seç...                ] │   │
│  │ ─────────────────────────────────────────────────  │   │
│  │ Eş │ Çocuk │ Anne │ Baba │ Kardeş │ Asistan │    │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  📝 NOT                                                     │
│  ┌─────────────────────────────────────────────────────┐   │
...
│  │  📱 Mobil                                          │   │
│  │  +90 532 397 94 00                                 │   │
│  │  ✓ Varsayılan • Son arama: Bugün 14:30            │   │
│  │                              [📞] [💬] [📋]       │   │
│  └─────────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  🏢 İş                                             │   │
...
│  📧 E-POSTA                                                │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  ahmet.yilmaz@example.com               Kişisel    │   │
│  │                                    [📧] [📋]       │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  🏢 KURULUŞ                                                │
...
│  🎂 ÖNEMLİ TARİHLER                                        │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  🎂 Doğum Günü: 15 Mart 1985 (39 yaşında)         │   │
...
│  │  ↗ 120 giden • ↙ 98 gelen • ↙ 26 cevapsız        │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  📝 NOTLAR                                                  │
│  ┌─────────────────────────────────────────────────────┐   │
...
│  ─────────────────────────────────────────────────────────  │
│                                                             │
│  [🚫 Engelle]    [🗑️ Sil]    [📤 Paylaş]                  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

#### 2.8.5 Hızlı Fotoğraf Atama

Kişi detay ekranında fotoğrafa dokunulduğunda:
...
│  │                                                     │   │
│  │  ┌────┐ ┌────┐ ┌────┐ ┌────┐                      │   │
│  │  │Cihaz│ │Googl│ │Grav │ │Özel │                     │   │
│  │  │Reh. │ │Cont.│ │atar │ │Foto │                     │   │
│  │  │ ✓  │ │    │ │    │ │    │                      │   │
│  │  └────┘ └────┘ └────┘ └────┘                      │   │
│  │                                                     │   │
│  │  Cihaz Rehberi fotoğrafı seçili                    │   │
│  │                                                     │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│                         [İptal]                             │
...
#### 2.8.6 TypeScript Interface'leri
...
type AccountType = 'google' | 'samsung' | 'phone' | 'whatsapp' | 'telegram' | 'microsoft' | 'other';

// Kişi veri modeli (kapsamlı)
interface Contact {
  // Tanımlayıcı
  id: string;
  rawContactId: string;
  ...
  familyName?: string;        // Soyad
  prefix?: string;            // Dr., Av.
  suffix?: string;            // Jr., PhD
  phoneticGivenName?: string;
  phoneticFamilyName?: string;
  nickname?: string;

  // Fotoğraf
  thumbnailPath?: string;     // Küçük resim
  photoUri?: string;          // Tam boyut
  hasPhoto: boolean;
  photoSource: 'device' | 'custom' | 'google' | 'gravatar' | 'avatar';
...
interface PhoneNumber {
  ...
  number: string;
  formattedNumber: string;
  label: 'mobile' | 'home' | 'work' | 'main' | 'fax_work' | 'fax_home' | 'pager' | 'other' | 'custom';
  customLabel?: string;
  isPrimary: boolean;
}
...
interface EmailAddress {
  id: string;
  email: string;
  label: 'personal' | 'work' | 'other' | 'custom';
  customLabel?: string;
  isPrimary: boolean;
}
...
interface Relation {
  id: string;
  name: string;
  contactId?: string;         // Bağlı kişi varsa
  type: 'spouse' | 'child' | 'mother' | 'father' | 'sibling' | 'assistant' | 'partner' | 'manager' | 'other' | 'custom';
  customType?: string;
}

// Ayarlardaki varsayılan hesap
interface ContactSettingsDefaults {
  defaultAccountId: string;       // Yeni kişiler bu hesaba eklenir
  defaultAccountType: AccountType;
  defaultAccountName: string;

  // Görünüm tercihleri
  visibleAccounts: string[];      // Gösterilecek hesap ID'leri
  sortBy: 'firstName' | 'lastName';
  displayFormat: 'firstLast' | 'lastFirst';
}
```
